...
Delegate to the `job-source-agent`. This is entirely optional — if you already have job postings from elsewhere, skip straight to `/nemo:apply --jobs-file <path>` with any file that roughly looks like a list of jobs. There's no schema link between what this command produces and what `/nemo:apply` accepts.
...
5. **De-duplicate** by posting/application URL before appending.
6. **Save.** Append entries to `.claude/nemohire/jobs/sourced.json` (a plain array — no id, no status; see `templates/tracker/job-cache-schema.md`), via `memory-agent`.
