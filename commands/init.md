...
3. **Structured interview.** Ask the user, in clearly grouped batches (not one question at a time), for whatever the resume (if found) didn't already answer:
   ...
   - EEO/demographic self-identification (all optional, explicitly say so when asking): gender, sexual orientation (mention this is asked by only a minority of forms), race/ethnicity, disability status, veteran status. Explain why up front — some US-based ATS forms (Greenhouse, Lever, Workday, etc.) ask these as a voluntary section, and having a real answer on file means `apply-agent` answers consistently instead of guessing or leaving it blank when the field is required. Any question left unanswered here is treated as "decline to self-identify" during `/nemohire:apply`, never guessed.
...
6. **Write files.** Populate every file under `./.claude/nemohire/identity/` (see `templates/identity/` for the exact schema of each file: profile.md, experience.md, achievements.md, skills.md, communication-style.md, writing-style.md, salary-preferences.md, job-preferences.md, company-preferences.md, location-preferences.md, work-authorisation.md, demographics-eeo.md, documents.md, interview-library.md, memory.md). Also create `./.claude/nemohire/config.md` from `templates/tracker/config.md` if it does not exist.
