...
- **Agents** (`agents/*.md`) — 4 single-responsibility subagents, each scoped to only the tools it actually uses:
  - **`apply-agent`** — the only agent that touches a job application. One dispatch per job, dispatched directly by `/nemohire:apply`/`/nemohire:continue` (no intermediate batching agent): opens it, writes any cover letter/answers it needs in your voice, fills, uploads, submits, and writes its own details file. It never touches the tracker or the ledger row itself — the dispatching command writes both from `apply-agent`'s returned outcome, right after it returns. Carries only Playwright MCP by default — the Chrome Connector and Gmail connector are deliberately excluded unless a specific job's `needs_input` outcome asks for one and you approve it.
...
There is exactly **one browser session, driven by one dispatch at a time** — `/nemohire:apply`/`/nemohire:continue` never have two `apply-agent` dispatches touching the browser together.
...
- Only one browsing dispatch is ever in flight at a time — the underlying browser session is shared and single-threaded by design.
...
- There is **no pre-submit summary or confirmation gate** — `apply-agent` submits directly once everything required has a real answer. The full record is written once, at the end, into `jobs/details/<seq>-<id>.md`'s Application record section for after-the-fact review.
...
1. For each job, one at a time, dispatched directly by this session — no intermediate batching agent: **dispatch `apply-agent`** with just `{id, seq, application_url}` — it never sees a posting or company context in full going in. It opens the application directly (there's nothing pre-written to read first), clicks through to the real form first if the page it lands on isn't already the form, decides whether it's a normal one-page form or one that needs manual handling (flagging it if so), writes a short company highlight and any cover letter/answers directly in your voice, fills, always attaches a resume when the form has anywhere to put one, and submits.
2. `apply-agent` writes `jobs/details/<seq>-<id>.md` for the first time — the whole file, every section, in one write — and returns a short outcome (status, note, date if submitted). It never touches the tracker or the ledger itself.
3. This session checkpoints the outcome as a single-line edit to that job's row in `jobs.jsonl`, and appends/updates its row in `tracker/applications.md` from the `co`/`role`/`url` it already had plus what `apply-agent` just returned — both right away, before moving to the next job.

One dispatch per job covers the whole thing — there's no hand-off between a browsing step and a separate content-writing step, and no separate agent or file used purely to coordinate between two layers. Minting ids happens once, up front, as a ledger-only edit — not repeated per job, and never involving `jobs/details/`.
