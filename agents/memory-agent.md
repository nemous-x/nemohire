...
description: >
  ...
  needs to read, append to, or mechanically update jobs/sourced.json, jobs/run-state.json,
  tracker files, cache files, or sync-state, without doing any reasoning about the content
  itself. Ids, cache seeding, and run-state checkpointing are all done by the active session
  running /nemo:apply or /nemo:continue directly (there is no coordinator subagent) — not this
  agent's job; it only performs the mechanical write when asked to.
...
- **`jobs/run-state.json`** (schema: `templates/tracker/run-state-schema.md`): when asked, write the initial `current_run` job list at the start of a run, update a single job's `status`/`outcome`/`note` as a checkpoint, or move `current_run` into `history` once a run finishes. You never decide a job's outcome yourself — you write exactly the status/outcome you're given by the active session running `/nemo:apply` or `/nemo:continue`.
