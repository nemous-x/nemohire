...
- Never duplicate a tracker entry for the same job id — update the existing row/line instead. Job ID is the reliable key (it matches `jobs.json` and the `jobs/applied/<id>/` folder); company+role is only a display fallback.
