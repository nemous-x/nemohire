...
name: file-upload
description: >
  Detects file upload fields on job application forms and attaches the correct prepared
  document (resume, cover letter, portfolio file) to each. Use during /nemo:apply once
  browser-agent has mapped the form's fields.
---
...
1. Take the field map produced by `browser-agent` and identify every upload-type field, its label, and accepted file types (PDF, DOCX, etc. — read `accept` attributes or visible instructions if available).
2. Match fields to files by purpose, not just by order on the page:
   - Resume/CV field → `jobs/prepared/<company>-<role>/resume.*`
   - Cover letter field → `jobs/prepared/<company>-<role>/cover-letter.*`
